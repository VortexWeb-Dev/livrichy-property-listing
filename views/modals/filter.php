...
<script>
    function prepareFilters(e) {
        ...
        const fieldMappings = {
            'city': '%ufCrm13City',
            'community': '%ufCrm13Community',
            'subCommunity': '%ufCrm13SubCommunity',
            'building': '%ufCrm13Tower',
            'reference': 'ufCrm13ReferenceNumber',
            'permit': 'ufCrm13ReraPermitNumber',
            'listing_title': '%ufCrm13TitleEn',
            'property_type': 'ufCrm13PropertyType',
            'saleRent': 'ufCrm13OfferingType',
            'listing_agent': '%ufCrm13AgentName',
            'developer': '%ufCrm13Developers',
            'listing_owner': '%ufCrm13ListingOwner',
            'landlordEmail': '%ufCrm13LandlordEmail',
            'landlordPhone': '%ufCrm13LandlordContact',
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const fetchAgents = async () => {
            try {
                const response = await fetch(`${baseUrl}/crm.item.list?entityTypeId=1050&select[0]=id&select[1]=ufCrm14AgentName&order[ufCrm14AgentName]=asc`);
        ...
        const fetchDevelopers = async () => {
            try {
                const response = await fetch(`${baseUrl}/crm.item.list?entityTypeId=1066&select[0]=id&select[1]=ufCrm18DeveloperName&order[ufCrm18DeveloperName]=asc`);
                const data = await response.json();
                const totalDevelopers = data.total;
                const developers = [];

                for (let i = 0; i < Math.ceil(totalDevelopers / 50); i++) {
                    const paginatedResponse = await fetch(`${baseUrl}/crm.item.list?entityTypeId=1066&select[0]=id&select[1]=ufCrm18DeveloperName&order[ufCrm18DeveloperName]=asc&start=${i * 50}`);
        ...
        const fetchAndDisplayOptions = async () => {
            try {
                ...
                createSelectOptions(developers, developerSelect, 'ufCrm18DeveloperName', 'ufCrm18DeveloperName');
