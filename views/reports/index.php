...
<script>
    window.addEventListener('load', async () => {
        ...
        let baseUrl = 'https://crm.livrichy.com/rest/1509/o8fnjtg7tyf787h4';
        let properties = [];
        const response = await fetch(`${baseUrl}/crm.item.list?entityTypeId=1046&select[0]=ufCrm13OfferingType&select[1]=ufCrm13PfEnable&select[2]=ufCrm13BayutEnable&select[3]=ufCrm13DubizzleEnable&select[4]=ufCrm13WebsiteEnable`);
        const data = await response.json();
        const totalOwners = data.total;

        for (let i = 0; i < Math.ceil(totalOwners / 50); i++) {
            const response = await fetch(`${baseUrl}/crm.item.list?entityTypeId=1046&select[0]=ufCrm13OfferingType&select[1]=ufCrm13PfEnable&select[2]=ufCrm13BayutEnable&select[3]=ufCrm13DubizzleEnable&select[4]=ufCrm13WebsiteEnable&start=${i * 50}`);
            const data = await response.json();
        ...
        properties.forEach(property => {
            if (property['ufCrm13OfferingType'] === 'RS' || property['offeringType'] === 'RR') {
                // Residential
                if (property['ufCrm13PfEnable']) residentialPropertyFinder++;
                if (property['ufCrm13BayutEnable']) residentialBayut++;
                if (property['ufCrm13DubizzleEnable']) residentialDubizzle++;
                if (property['ufCrm13WebsiteEnable']) residentialWebsite++;
                if (property['ufCrm13OfferingType'] === 'RS') {
                    residentialSale++;
                } else {
                    residentialRent++;
                }
            } else if (property['ufCrm13OfferingType'] === 'CS' || property['offeringType'] === 'CR') {
                // Commercial
                if (property['ufCrm13PfEnable']) commercialPropertyFinder++;
                if (property['ufCrm13BayutEnable']) commercialBayut++;
                if (property['ufCrm13DubizzleEnable']) commercialDubizzle++;
                if (property['ufCrm13WebsiteEnable']) commercialWebsite++;
                if (property['ufCrm13OfferingType'] === 'CS') {
                    commercialSale++;
                } else {
                    commercialRent++;
                }
